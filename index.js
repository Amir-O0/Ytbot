...
// Set up the bot with the token from the BotFather
const bot = new TelegramBot("5696293826:AAGNv2K-XTrFxNkmjbB9EnMr46g27dxbU3w", {
  polling: true,
});
...
// Listen for messages starting with '/video'
bot.onText(/\/video (.+)/, async (msg, match) => {
  ...
  const API_KEY1 = "AIzaSyCu2J0WlTi1FLQ5x-FmETxejZ8dAlHulVw";
  ...
  // Get the video title and send it to the user
  const videoTitle = videoDetails.data.items[0].snippet.title;
  bot.sendMessage(chatId, `Sending video: ${videoTitle}`);
